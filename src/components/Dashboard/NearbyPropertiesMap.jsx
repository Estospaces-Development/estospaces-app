...
import { MapPin, Home, X, Navigation, ZoomIn, ZoomOut } from 'lucide-react';
import { calculateDistance } from '../../services/locationService';

/**
 * Nearby Properties Map Component
...
const NearbyPropertiesMap = ({ 
  ...
  const getMarkerColor = (category) => {
    switch (category) {
      case 'very-near':
        return 'bg-green-500'; // Very close (within 1 mile)
      case 'near':
        return 'bg-orange-500'; // Near (1-3 miles)
      case 'moderate':
        return 'bg-yellow-500'; // Moderate (3-5 miles)
      case 'far':
        return 'bg-gray-400'; // Far (5+ miles)
      default:
        return 'bg-blue-500';
    }
  };
  ...
  const latLngToPosition = (lat, lng, bounds) => {
    if (!bounds) {
      // Simple positioning if no bounds
      ...
      {selectedProperty && (
        <div className="absolute top-4 right-4 w-80 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 p-4 z-30 max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-2">
            ...
            {selectedProperty.distance !== null && (
