...
const ProfileCompletionCard = () => {
    ...
    const isProfileIncomplete = useMemo(() => {
        ...
        const hasName = profile?.full_name || user?.user_metadata?.full_name;
        const hasPhone = profile?.phone || user?.user_metadata?.phone;

        return !hasName || !hasPhone;
    }, [profile, user, loading]);

    // DEMO: Force visibility for user review
    if (!isVisible) return null;

    return (
        <div id="profile-widget" className="z-50 animate-slideInRight">
