...
const ChatList = ({ brokers, selectedBroker, onSelectBroker }) => {
  return (
            ...
            {brokers.map((broker) => (
              <button
                ...
                className={`w-full p-4 text-left hover:bg-gray-50 transition-colors ${
                  selectedBroker?.id === broker.id ? 'bg-orange-50 border-l-4 border-orange-500' : ''
                }`}
              >
                  ...
                  <div className="relative flex-shrink-0">
                    ...
                    <div
                      className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2 border-white ${
                        broker.isOnline ? 'bg-green-500' : 'bg-gray-400'
                      }`}
                    />
                  </div>
