...
const ApplicationDetail = ({ applicationId, application: initialApplication, onClose, onUpdateStatus }) => {
  ...
  const canWithdraw = ![APPLICATION_STATUS.WITHDRAWN, APPLICATION_STATUS.APPROVED, APPLICATION_STATUS.REJECTED].includes(application.status);
  ...
  const getActivityHistory = () => {
    ...
    if (application.status !== APPLICATION_STATUS.PENDING && application.status !== APPLICATION_STATUS.SUBMITTED) {
      history.push({
        ...
        date: new Date(new Date(application.submittedDate).getTime() + 24 * 60 * 60 * 1000).toISOString(),
        icon: Clock,
        color: 'orange'
      });
    }
    ...
    if (application.status === APPLICATION_STATUS.DOCUMENTS_REQUESTED) {
      history.push({
        ...
        date: new Date(new Date(application.submittedDate).getTime() + 2 * 24 * 60 * 60 * 1000).toISOString(),
        icon: AlertCircle,
        color: 'yellow'
      });
    }
