...
/**
 * PropertyFilterTabs Component
 * Filter tabs with "Browse Properties" dropdown containing Buy and Rent options
 * 
 * @param {string} activeTab - Current active tab: 'all' | 'buy' | 'rent'
 * @param {function} onTabChange - Callback when tab changes: (tab: 'all' | 'buy' | 'rent') => void
 * @param {boolean} loading - Loading state
 */
const PropertyFilterTabs = ({ activeTab = 'all', onTabChange, loading = false }) => {
  ...
  const handleBrowseClick = () => {
    // Always toggle dropdown when clicking Browse Properties
  ...
  const getActiveLabel = () => {
    // Always show "Browse Properties" as the main label
    return 'Browse Properties';
  };

  const getActiveIcon = () => {
    // Show the icon of the active filter, or Search for 'all'
  ...
  // Button is always active when any filter is selected
  ...
  return (
    <div className="flex items-center gap-2 border-b border-gray-200 dark:border-gray-700 mb-6">
      {/* Browse Properties Dropdown Button */}
      <div className="relative" ref={dropdownRef}>
        ...
        {/* Dropdown Menu - Always show when open, regardless of active tab */}
        {isDropdownOpen && (
          <div className="absolute top-full left-0 mt-1 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-50 overflow-hidden">
            {/* Show All option if a filter is active */}
            {activeTab !== 'all' && (
              <button
                onClick={handleResetToAll}
                className="w-full flex items-center gap-3 px-4 py-3 text-sm text-gray-700 dark:text-gray-300 hover:bg-orange-50 dark:hover:bg-orange-900/20 hover:text-orange-600 dark:hover:text-orange-400 transition-colors border-b border-gray-200 dark:border-gray-700"
              >
