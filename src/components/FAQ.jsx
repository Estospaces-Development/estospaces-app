...
const FAQ = () => {
    ...
    return (
        <section id="faq" className="py-32 bg-gradient-to-b from-white via-gray-50 to-white relative overflow-hidden">
            ...
            <div className="container mx-auto px-4 relative z-10">
                ...
                <div className="max-w-4xl mx-auto space-y-5">
                    {faqs.map((faq, index) => (
                        <div
                            key={index}
                            className={`bg-white border-2 rounded-2xl overflow-hidden transition-all duration-300 shadow-md hover:shadow-xl ${openIndex === index ? 'border-primary shadow-lg' : 'border-gray-100'
                                }`}
                        >
                            <button
                            ...
                            >
                                <span className={`font-bold text-xl pr-8 transition-colors ${openIndex === index ? 'text-primary' : 'text-secondary group-hover:text-primary'
                                    }`}>
                                    {faq.question}
                                </span>
                                <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${openIndex === index ? 'bg-primary rotate-180' : 'bg-gray-100 group-hover:bg-primary/10'
                                    }`}>
                                    <ChevronDown
