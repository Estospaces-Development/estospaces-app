...
const CommunityPostCard: React.FC<CommunityPostCardProps> = ({
    ...
    return (
        <div
            className={`bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all duration-300 ${post.isPinned ? 'ring-2 ring-primary/30' : ''
                }`}
        >
            {/* Header */}
            <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <span
                                className={`px-2 py-0.5 rounded-md text-xs font-medium ${getRoleBadgeColors(
                                    post.authorRole
                                )}`}
                            >
                ...
                <div className="flex items-center gap-2">
                    {/* Tag */}
                    <span
                        className={`px-3 py-1 rounded-lg text-xs font-semibold uppercase ${getTagColors(
                            post.tag
                        )}`}
                    >
                        {post.tag}
                    ...
                    {isManager && (
                        <div className="relative">
                            ...
                            {showActions && (
                                <div className="absolute right-0 top-8 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg py-1 z-10 w-40">
                                    <button
                                        onClick={() => {
                                            onPin(post.postId);
                                    ...
                                    <button
                                        onClick={() => {
                                            onVisibilityChange(post.postId, 'all');
                                            setShowActions(false);
                                        }}
                                    ...
                                    <button
                                        onClick={() => {
                                            onVisibilityChange(post.postId, 'managers');
                                            setShowActions(false);
                                        }}
                                        className="w-full px-4 py-2 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
                                    >
                                    ...
                                    <button
                                        onClick={() => {
                                            onVisibilityChange(post.postId, 'brokers');
                                            setShowActions(false);
                                        }}
                                        className="w-full px-4 py-2 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
                                    >
            ...
            <div className="flex items-center gap-4 pt-3 border-t border-gray-200 dark:border-gray-700">
                ...
                <button
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 bg-gray-100 dark:bg-gray-700 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors cursor-pointer"
                >
