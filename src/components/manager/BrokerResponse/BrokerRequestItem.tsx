...
const BrokerRequestItem: React.FC<BrokerRequestItemProps> = ({ request, onRespond }) => {
  ...
  // For mock purposes, we'll just track the countdown state locally starting from a fixed value 
  // if it's pending, to ensure the UI demo works smoothly.
  // In a real app, this would be: 600 - (now - timestamp) / 1000
  ...
  // Handle immediate status change from props if parent updates it (e.g. mock data refresh)
  useEffect(() => {
  ...
  const handleRespond = (e: React.MouseEvent) => {
    ...
    navigate('/manager/dashboard/messages'); // Redirect to messages
  };

  const handleViewProperty = (e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent modal opening
    navigate(`/manager/dashboard/broker-property/${request.id}`); // Redirect to mock property page
  };
  ...
  const getProgressColor = () => {
    if (secondsRemaining < 60) return 'bg-red-500'; // Last minute
    if (secondsRemaining < 300) return 'bg-yellow-500'; // Last 5 mins
    return 'bg-green-500';
  };
      ...
      >
        <div className="flex justify-between items-start mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-semibold text-gray-900 dark:text-white text-sm hover:text-primary transition-colors">{request.brokerName}</h4>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2 mt-3">
              <button
                ...
                className="flex-1 bg-primary hover:bg-primary-dark text-white text-xs font-medium py-2 px-3 rounded-md transition-colors flex items-center justify-center gap-1 group relative z-10"
              >
