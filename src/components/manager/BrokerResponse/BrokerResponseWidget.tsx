...
const BrokerResponseWidget: React.FC = () => {
    ...
    // Listen for user requests from EmergencyWidget
    useEffect(() => {
        const handleStorageChange = (e: StorageEvent) => {
            if (e.key === 'estospaces_broker_request') {
                addNewRequest(e.newValue);
            }
        };

        window.addEventListener('storage', handleStorageChange);

        // Poll for same-window testing
        const interval = setInterval(() => {
            const req = localStorage.getItem('estospaces_broker_request');
            if (req) {
                // Check if we already have this ID to avoid dupes (simple check)
                const requestData = JSON.parse(req);
                setRequests(prev => {
                    if (prev.some(r => r.id === requestData.id)) return prev;
                    return [
    ...
    const addNewRequest = (jsonString: string | null) => {
        if (!jsonString) return;
        try {
            const newReq = JSON.parse(jsonString);
            setRequests(prev => {
                if (prev.some(r => r.id === newReq.id)) return prev;
                return [
                    { ...newReq, timestamp: new Date(newReq.timestamp) },
                    ...prev
                ];
            });
            // Clear storage so we don't re-add endlessly if logic fails, though ID check prevents it
        } catch (e) {
    ...
    const handleRespond = (id: string) => {
        setRequests(prev => prev.map(req => {
            if (req.id === id) {
                // If this was a user request, notify them back
                if (req.propertyName === 'User Location Request') {
                    ...
                    localStorage.setItem('estospaces_broker_response', JSON.stringify(responseData));
        ...
        <div className="bg-white dark:bg-black rounded-lg shadow-sm border border-gray-200 dark:border-gray-800 p-6 ring-2 ring-blue-500/10">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h3 className="section-heading text-gray-800 dark:text-white flex items-center gap-2">
            ...
            <div className="mt-6 flex justify-between items-center text-xs text-gray-500 border-t border-gray-100 dark:border-gray-800 pt-4">
                ...
                <button
                    onClick={() => navigate('/manager/dashboard/broker-requests')}
                    className="flex items-center gap-1 text-primary hover:text-primary-dark font-medium group"
                >
                    View All Requests
