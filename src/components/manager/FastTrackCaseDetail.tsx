...
const FastTrackCaseDetail: React.FC<FastTrackCaseDetailProps> = ({ caseData, onClose, onUpdate }) => {
    ...
    const handleSignRequest = (id: number) => {
        setEsignDocs(prev => prev.map(doc => doc.id === id ? { ...doc, status: 'sent' } : doc));
    };
    ...
    const toggleDocument = (docType: keyof typeof caseData.documents) => {
        ...
        // Auto-advance step if all docs verified (mock logic)
        let newStep = caseData.currentStep;
        if (newStatus === 'verified' && Object.values({ ...updatedDocs, [docType]: newStatus }).every(s => s === 'verified') && caseData.currentStep === 'documents') {
            // In a real app we might ask confirmation, here we just show it's ready
        }
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8">
                    ...
                    {activeTab === 'esignature' && (
                        <div className="bg-white dark:bg-black border border-gray-200 dark:border-zinc-800 rounded-2xl p-6">
                            ...
                            <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-900/30 flex gap-3">
                                <Clock className="text-blue-600 dark:text-blue-400 flex-shrink-0" size={20} />
                                <div>
                                    <h4 className="font-semibold text-blue-800 dark:text-blue-400 text-sm">Audit Trail</h4>
                                    <p className="text-xs text-blue-600 dark:text-blue-300 mt-1">
                                        All actions are timestamped and logged securely. Last activity: Document viewed by client 10 mins ago.
                                    </p>
                                </div>
                            </div>
                        </div>
