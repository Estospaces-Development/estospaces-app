...
const SharePropertyModal = ({ isOpen, onClose, property, onShare }: SharePropertyModalProps) => {
  ...
  const url = `${window.location.origin}/manager/dashboard/properties/${property.id}`;
  const propertyImage = property.media?.images?.[0]?.url || property.images?.[0] || '';
  ...
  const location = property.location 
    ? `${property.location.city || ''}${property.location.city && property.location.state ? ', ' : ''}${property.location.state || ''}${(property.location.city || property.location.state) && property.location.country ? ', ' : ''}${property.location.country || ''}`
    : '';
  
  const description = property.description?.slice(0, 200) || `${property.title} - ${price}${location ? ` in ${location}` : ''}`;
  ...
  const handleShare = async (platform: string, e?: React.MouseEvent) => {
    ...
    try {
      switch (platform) {
        ...
        case 'facebook': {
          const shareText = `${property.title} - ${price}${location ? ` in ${location}` : ''}`;
          const shareUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}&quote=${encodeURIComponent(shareText)}`;
          const shareWindow = window.open(shareUrl, 'facebook-share-dialog', 'width=626,height=436,menubar=no,toolbar=no,resizable=yes,scrollbars=yes');
          if (!shareWindow) {
            showToast('Please allow popups to share on Facebook', 'error');
          } else {
        ...
        case 'twitter': {
          const text = `${property.title} - ${price}${location ? ` in ${location}` : ''}\n${url}`;
          const shareUrl = `https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${encodeURIComponent(text)}`;
          const shareWindow = window.open(shareUrl, 'twitter-share-dialog', 'width=550,height=420,menubar=no,toolbar=no,resizable=yes,scrollbars=yes');
          if (!shareWindow) {
            showToast('Please allow popups to share on Twitter', 'error');
          } else {
        ...
        case 'linkedin': {
          ...
          const shareWindow = window.open(shareUrl, 'linkedin-share-dialog', 'width=520,height=570,menubar=no,toolbar=no,resizable=yes,scrollbars=yes');
          if (!shareWindow) {
            showToast('Please allow popups to share on LinkedIn', 'error');
          } else {
        ...
        case 'whatsapp': {
          ...
          if (!shareWindow) {
            showToast('Please allow popups to share on WhatsApp', 'error');
          } else {
                  ...
                  <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className={`px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 cursor-pointer active:scale-95 ${
                          copied
